...
const API_BASE_URL = 'http://192.168.1.57:3001/api';
  ...
  if (history) {
    for (const item of history) {
      ...
      if (item.role === 'user' || item.role === 'assistant') {
        parsedHistory.push([item.role === "user" ? "human" : "assistant", item.content.trim()]);
      }
    }
  }  
...
export const getSuggestions = async (
  ...
  return fetchApi<Suggestion>('/suggestions', {
    method: 'POST',
    body: JSON.stringify({
      chat_history: history?.map((item) => ({ role: item.role, content: item.content + ". Réponds uniquement dans la même langue que la requête." }))
    }),
  });
};
